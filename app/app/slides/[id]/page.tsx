...
function highlightGlossary(root: HTMLElement, terms: GlossaryTerm[]) {
  const pattern = new RegExp(`(${terms.map((item) => item.term.trim()).filter(Boolean).sort((a, b) => b.length - a.length).map((term) => term.replace(/[.*+?^${}()|[\\]\\]/g, "\\$&")).join("|")})`, "gi");
  if (!pattern.source || pattern.source === "()") return;
  ...
  useEffect(() => {
    ...
    const render = async () => {
      try {
        ...
        if (!cancelled) { setStatus(""); if (glossary?.length) highlightGlossary(previewRef.current!, glossary); }
