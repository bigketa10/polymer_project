...
export default function SlidePresentationPage() {
  ...
  useEffect(() => {
    ...
    const render = async () => {
      ...
      try {
        ...
        if (!response.ok)
          throw new Error(
            "The stored PowerPoint file could not be downloaded.",
          );
        ...
        const presentationXml = await zip
          ...
          ?.async("string");
        const sizeMatch = presentationXml?.match(
          /<p:sldSz[^>]*cx="(\d+)"[^>]*cy="(\d+)"/,
        );
        ...
        if (!previewRef.current.children.length) {
          throw new Error(
            "The PowerPoint renderer produced no slides. Try exporting the deck as PDF.",
          );
        }
  ...
  if (!deck)
    return (
      <main className="p-8">
        <p>Deck not found.</p>
        <Link className="text-blue-600" href="/teacher/slides">
          Back to decks
        </Link>
      </main>
    );
      ...
      <div className="mx-auto max-w-7xl pb-8">
        ...
        {fileUrl && (
          <a
            ...
            className="mb-4 inline-flex items-center rounded-md bg-sky-300 px-3 py-2 text-sm font-semibold text-slate-950 hover:bg-sky-200"
          >
        ...
        {renderError && (
          <div className="mb-4 rounded border border-red-400/40 bg-red-950/40 p-4 text-sm text-red-100">
            ...
            <p className="mt-2 text-red-200">
              The original PowerPoint is stored, but this browser renderer could
              not display it.
            </p>
          </div>
        )}
